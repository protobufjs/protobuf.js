...
.forEach(function({ file, flags, out }) {
    ...
    pbjs.main([
        ...
        fs.writeFileSync(out, output.replace(/"@pgherveou\/protobufjs\/minimal"/g, JSON.stringify(pathToProtobufjs)));
...
.forEach(function({ file, ext }) {
    ...
    pbts.main(args.concat(file), function(err, output) {
        ...
        output = output.replace(/"@pgherveou\/protobufjs"/g, JSON.stringify(pathToProtobufjs));
