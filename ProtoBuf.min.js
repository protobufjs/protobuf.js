...
*/
(function(p){function q(n){if(!n||!n.zigZagEncode32)throw Error("ProtoBuf.js requires ByteBuffer.js >=1.1.0: Get it at https://github.com/dcodeIO/ByteBuffer.js");var f={VERSION:"0.12.11",WIRE_TYPES:{}};f.WIRE_TYPES.VARINT=0;f.WIRE_TYPES.BITS64=1;f.WIRE_TYPES.LDELIM=2;f.WIRE_TYPES.STARTGROUP=3;f.WIRE_TYPES.ENDGROUP=4;f.WIRE_TYPES.BITS32=5;f.TYPES={int32:{name:"int32",wireType:f.WIRE_TYPES.VARINT},uint32:{name:"uint32",wireType:f.WIRE_TYPES.VARINT},sint32:{name:"sint32",wireType:f.WIRE_TYPES.VARINT},
...
c){this.parent=a;this.name=c;this.resolvedType=null};f.prototype.toString=function(c){var l=this.name,d=this;do{d=d.parent;if(null==d)break;l=d.name+"."+l}while(1);c&&(this instanceof b.Message?l="Message "+l:this instanceof a.Field?l="Message.Field "+l:this instanceof e?l="Enum "+l:this instanceof e.Value?l="Enum.Value "+l:this instanceof h&&(l="Namespace "+l));return l};f.prototype.build=function(){throw Error(this.toString(!0)+" cannot be built directly");};b.T=f;var h=function(a,c,b){f.call(this,
...
if(!(g instanceof a.Message.Field))throw Error(this+"#"+c+" is not a field: "+g.toString(!0));this[g.name]=g.verifyValue(d)};g.prototype.get=function(c){var g=b.getChild(c);if(!g||!(g instanceof a.Message.Field))throw Error(this+"#"+c+" is not a field: undefined");if(!(g instanceof a.Message.Field))throw Error(this+"#"+c+" is not a field: "+g.toString(!0));return this[g.name]};for(var l=0;l<d.length;l++)(function(a){var c=a.name.replace(/(_[a-zA-Z])/g,function(a){return a.toUpperCase().replace("_",
"")}),c=c.substring(0,1).toUpperCase()+c.substring(1),d=a.name.replace(/([A-Z])/g,function(a){return"_"+a});b.hasChild("set"+c)||(g.prototype["set"+c]=function(b){this.set(a.name,b)});b.hasChild("set_"+d)||(g.prototype["set_"+d]=function(b){this.set(a.name,b)});b.hasChild("get"+c)||(g.prototype["get"+c]=function(){return this.get(a.name)});b.hasChild("get_"+d)||(g.prototype["get_"+d]=function(){return this.get(a.name)})})(d[l]);g.prototype.encode=function(a){a=a||new n;var c=a.littleEndian,g=b.encode(this,
a.LE()).flip();a.littleEndian=c;return g};g.prototype.toArrayBuffer=function(){return this.encode().toArrayBuffer()};g.decode=function(a){a=a?a instanceof n?a:n.wrap(a):new n;var c=a.littleEndian,g=b.decode(a.LE());a.littleEndian=c;return g};g.prototype.toString=function(){return b.toString()};Object.defineProperty&&Object.defineProperty(g,"$options",{value:b.buildOpt(),enumerable:!1,configurable:!1,writable:!1});return g}(b,this),l=this.getChildren(),k=0;k<l.length;k++)if(l[k]instanceof e)d[l[k].name]=
l[k].build();else if(l[k]instanceof a)d[l[k].name]=l[k].build();else if(!(l[k]instanceof a.Field))throw Error("Illegal reflect child of "+this.toString(!0)+": "+l[k].toString(!0));return this.built=d};a.prototype.encode=function(b,c){for(var d=this.getChildren(a.Field),e=0;e<d.length;e++)d[e].encode(b.get(d[e].name),c);return c};a.prototype.decode=function(a,c){c=c||-1;for(var d=a.offset,e=new this.built;a.offset<d+c||-1==c&&0<a.remaining();){var f=a.readVarint32(),h=f&7,f=f>>3,m=this.getChild(f);
if(!m)throw Error("Illegal field id in "+this.toString(!0)+"#decode: "+f);m.repeated&&!m.options.packed?e.add(m.name,m.decode(h,a)):e.set(m.name,m.decode(h,a))}d=this.getChildren(b.Field);for(h=0;h<d.length;h++)if(d[h].required&&null===e[d[h].name])throw d=Error("Missing field "+d[h].toString(!0)+" in "+this.toString(!0)+"#decode"),d.msg=e,d;return e};b.Message=a;var d=function(a,b,c,d,e,h){f.call(this,a,d);this.required="required"==b;this.repeated="repeated"==b;this.type=c;this.id=e;this.options=
h||{}};d.prototype=Object.create(f.prototype);d.prototype.verifyValue=function(a,b){b=b||!1;if(null===a){if(this.required)throw Error("Illegal value for "+this.toString(!0)+": "+a+" (required)");return null}var d,f;if(this.repeated&&!b){a instanceof Array||(a=[a]);f=[];for(d=0;d<a.length;d++)f.push(this.verifyValue(a[d],!0));return f}if(!this.repeated&&a instanceof Array)throw Error("Illegal value for "+this.toString(!0)+": "+a+" (no array expected)");if(this.type==c.TYPES.int32||this.type==c.TYPES.sint32||
...
this.resolvedType.encode(a,b);var e=b.offset-d-1,f=n.calculateVarint32(e);if(1<f){var h=b.clone().flip();h.offset=h.length-e;b.append(h,d+f)}b.writeVarint32(e,d)}else throw Error("[INTERNAL ERROR] Illegal value to encode in "+this.toString(!0)+": "+a+" (unknown type)");return b}};d.prototype.decode=function(a,b,d){if(a!=this.type.wireType&&(d||a!=c.WIRE_TYPES.LDELIM||!this.repeated))throw Error("Illegal wire type for field "+this.toString(!0)+": "+a+" ("+this.type.wireType+" expected)");if(a==c.WIRE_TYPES.LDELIM&&
...
g.name+": "+JSON.stringify(e.fields[k]));if(g.hasChild(e.fields[k].id))throw Error("Duplicate field id in message "+g.name+": "+e.fields[k].id);if(e.fields[k].options){l=Object.keys(e.fields[k].options);for(n=0;n<l.length;n++){if(!b.NAME.test(l[n]))throw Error("Illegal field option name in message "+g.name+"#"+e.fields[k].name+": "+l[n]);if(!b.ID.test(""+e.fields[k].options[l[n]])&&!b.TYPEREF.test(e.fields[k].options[l[n]]))throw Error("Illegal field option value in message "+g.name+"#"+e.fields[k].name+
"#"+l[n]+": "+e.fields[k].options[l[n]]);}}g.addChild(new f.Message.Field(g,e.fields[k].rule,e.fields[k].type,e.fields[k].name,e.fields[k].id,e.fields[k].options))}l=[];if("undefined"!=typeof e.enums&&0<e.enums.length)for(k=0;k<e.enums.length;k++)l.push(e.enums[k]);if(e.messages&&0<e.messages.length)for(k=0;k<e.messages.length;k++)l.push(e.messages[k]);this.ptr.addChild(g);0<l.length&&(c.push(a),a=l,this.ptr=g)}else if(h.isValidEnum(e)){g=new f.Enum(this.ptr,e.name,e.options);for(k=0;k<e.values.length;k++)g.addChild(new f.Enum.Value(g,
e.values[k].name,e.values[k].id));this.ptr.addChild(g)}else throw Error("Not a valid message or enum definition: "+JSON.stringify(e));else throw Error("Not a valid namespace definition: "+JSON.stringify(a));this.ptr=this.ptr.parent}this.resolved=!1;this.result=null;return this}};h.prototype["import"]=function(a,b){a["package"]&&this.define(a["package"],a.options);a.messages&&this.create(a.messages);this.reset();a["package"]&&this.define(a["package"],a.options);a.enums&&this.create(a.enums);this.reset();
if(a.imports&&0<a.imports.length){if(!b)throw Error("Cannot determine import root: File name is unknown");for(var e=b.replace(/[\/\\][^\/\\]*$/,""),f=0;f<a.imports.length;f++){var h=e+"/"+a.imports[f];if(/\.json$/i.test(h)){var k=c.Util.fetch(h);if(null===k)throw Error("Failed to import '"+h+"' in '"+b+"': File not found");this["import"](JSON.parse(k),h)}else if(!/google\/protobuf\//.test(h)){k=c.Util.fetch(h);if(null===k)throw Error("Failed to import '"+h+"' in '"+b+"': File not found");k=new c.DotProto.Parser(k+
"");this["import"](k.parse(),h)}}}return this};h.prototype.resolveAll=function(){if(!(null==this.ptr||"object"==typeof this.ptr.type)){if(this.ptr instanceof f.Namespace)for(var a=this.ptr.getChildren(),d=0;d<a.length;d++)this.ptr=a[d],this.resolveAll();else if(this.ptr instanceof f.Message.Field)if(b.TYPE.test(this.ptr.type))this.ptr.type=c.TYPES[this.ptr.type];else{if(!b.TYPEREF.test(this.ptr.type))throw Error("Illegal type reference in "+this.ptr.toString(!0)+": "+this.ptr.type);a=this.ptr.parent.resolve(this.ptr.type);
if(!a)throw Error("Unresolvable type reference in "+this.ptr.toString(!0)+": "+this.ptr.type);this.ptr.resolvedType=a;if(a instanceof f.Enum)this.ptr.type=c.TYPES["enum"];else if(a instanceof f.Message)this.ptr.type=c.TYPES.message;else throw Error("Illegal type reference in "+this.ptr.toString(!0)+": "+this.ptr.type);}else if(!(this.ptr instanceof c.Reflect.Enum.Value))throw Error("Illegal object type in namespace: "+typeof this.ptr+":"+this.ptr);this.reset()}};h.prototype.build=function(a){this.reset();
