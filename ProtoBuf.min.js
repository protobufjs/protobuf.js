...
if(null===d)throw Error("Unexpected EOF in "+c.name+", "+a+" (ignored)");if(d==b.END)break}while(1)};c.prototype._parseMessage=function(c,a){var d={};a=this.tn.next();if(!b.NAME.test(a))throw Error("Illegal message name"+(c?" in message "+c.name:"")+": "+a);d.name=a;a=this.tn.next();if(a!=b.OPEN)throw Error("Illegal OPEN after message "+d.name+": "+a+" ('"+b.OPEN+"' expected)");d.fields=[];d.enums=[];d.messages=[];d.options={};do if(a=this.tn.next(),a==b.CLOSE)break;else if(b.RULE.test(a))this._parseMessageField(d,
a);else if("enum"==a)this._parseEnum(d,a);else if("message"==a)this._parseMessage(d,a);else if("option"==a)this._parseOption(d,a);else if("extensions"==a)this._parseIgnoredStatement(d,a);else throw Error("Illegal token in message "+d.name+": "+a+" (type or '"+b.CLOSE+"' expected)");while(1);c.messages.push(d);return d};c.prototype._parseMessageField=function(c,a){var d={};d.rule=a;a=this.tn.next();if(!b.TYPE.test(a)&&!b.TYPEREF.test(a))throw Error("Illegal field type in message "+c.name+": "+a);d.type=
a;a=this.tn.next();if(!b.NAME.test(a))throw Error("Illegal field name in message "+c.name+": "+a);d.name=a;a=this.tn.next();if(a!=b.EQUAL)throw Error("Illegal field number operator in message "+c.name+"#"+d.name+": "+a+" ('"+b.EQUAL+"' expected)");a=this.tn.next();if(!b.ID.test(a))throw Error("Illegal field number in message "+c.name+"#"+d.name+": "+a);d.id=parseInt(a,10);d.options={};a=this.tn.next();a==b.OPTOPEN&&(this._parseFieldOptions(c,d,a),a=this.tn.next());if(a!=b.END)throw Error("Illegal field delimiter in message "+
...
c.name+"#"+a.name+": "+d);var g=d;d=this.tn.next();if(e){if(d!=b.COPTCLOSE)throw Error("Illegal custom field option name delimiter in message "+c.name+"#"+a.name+": "+d+" (')' expected)");d=this.tn.next()}if(d!=b.EQUAL)throw Error("Illegal field option operation in message "+c.name+"#"+a.name+": "+d+" ('=' expected)");d=this.tn.next();if(d==b.STRINGOPEN){if(e=this.tn.next(),d=this.tn.next(),d!=b.STRINGCLOSE)throw Error("Illegal end of field value in message "+c.name+"#"+a.name+", option "+g+": "+
d+" ('"+b.STRINGCLOSE+"' expected)");}else if(b.NUMBER.test(d))e=parseFloat(d);else if(b.TYPEREF.test(d))e=d;else throw Error("Illegal field option value in message "+c.name+"#"+a.name+", option "+g+": "+d);a.options[g]=e};c.prototype._parseEnum=function(c,a){var d={};a=this.tn.next();if(!b.NAME.test(a))throw Error("Illegal enum name in message "+c.name+": "+a);d.name=a;a=this.tn.next();if(a!=b.OPEN)throw Error("Illegal OPEN after enum "+d.name+": "+a);d.values=[];d.options={};do{a=this.tn.next();
if(a==b.CLOSE)break;if("option"==a)this._parseOption(d,a);else{if(!b.NAME.test(a))throw Error("Illegal enum value name in enum "+d.name+": "+a);this._parseEnumValue(d,a)}}while(1);c.enums.push(d)};c.prototype._parseEnumValue=function(c,a){var d={};d.name=a;a=this.tn.next();if(a!=b.EQUAL)throw Error("Illegal enum value operator in enum "+c.name+": "+a+" ('"+b.EQUAL+"' expected)");a=this.tn.next();if(!b.ID.test(a))throw Error("Illegal enum value value in enum "+c.name+": "+a);d.id=parseInt(a,10);c.values.push(d);
a=this.tn.next();a==b.OPTOPEN&&(this._parseFieldOptions(c,{options:{}},a),a=this.tn.next());if(a!=b.END)throw Error("Illegal enum value delimiter in enum "+c.name+": "+a+" ('"+b.END+"' expected)");};c.prototype.toString=function(){return"Parser"};return c}(f,f.Lang,f.DotProto.Tokenizer);f.Reflect=function(c){var b={},f=function(a,c){this.parent=a;this.name=c;this.resolvedType=null};f.prototype.toString=function(c){var l=this.name,d=this;do{d=d.parent;if(null==d)break;l=d.name+"."+l}while(1);c&&(this instanceof
b.Message?l="Message "+l:this instanceof a.Field?l="Message.Field "+l:this instanceof e?l="Enum "+l:this instanceof e.Value?l="Enum.Value "+l:this instanceof h&&(l="Namespace "+l));return l};f.prototype.build=function(){throw Error(this.toString(!0)+" cannot be built directly");};b.T=f;var h=function(a,c,b){f.call(this,a,c);this.children=[];this.options=b||{}};h.prototype=Object.create(f.prototype);h.prototype.getChildren=function(a){a=a||null;if(null==a)return this.children.slice();for(var c=[],
...
function(c){var d=b.getChild(c);if(!d||!(d instanceof a.Message.Field))throw Error(this+"#"+c+" is not a field: undefined");if(!(d instanceof a.Message.Field))throw Error(this+"#"+c+" is not a field: "+d.toString(!0));return this[d.name]};for(var l=0;l<d.length;l++)(function(a){var c=a.name.replace(/(_[a-zA-Z])/g,function(a){return a.toUpperCase().replace("_","")}),c=c.substring(0,1).toUpperCase()+c.substring(1),d=a.name.replace(/([A-Z])/g,function(a){return"_"+a});b.hasChild("set"+c)||(g.prototype["set"+
c]=function(b){this.set(a.name,b)});b.hasChild("set_"+d)||(g.prototype["set_"+d]=function(b){this.set(a.name,b)});b.hasChild("get"+c)||(g.prototype["get"+c]=function(){return this.get(a.name)});b.hasChild("get_"+d)||(g.prototype["get_"+d]=function(){return this.get(a.name)})})(d[l]);g.prototype.encode=function(a){a=a||new n;var c=a.littleEndian,d=b.encode(this,a.LE()).flip();a.littleEndian=c;return d};g.prototype.toArrayBuffer=function(){return this.encode().toArrayBuffer()};g.decode=function(a){a=
a?a instanceof n?a:n.wrap(a):new n;var c=a.littleEndian,d=b.decode(a.LE());a.littleEndian=c;return d};g.prototype.toString=function(){return b.toString()};Object.defineProperty&&Object.defineProperty(g,"$options",{value:b.buildOpt(),enumerable:!1,configurable:!1,writable:!1});return g}(b,this),l=this.getChildren(),k=0;k<l.length;k++)if(l[k]instanceof e)d[l[k].name]=l[k].build();else if(l[k]instanceof a)d[l[k].name]=l[k].build();else if(!(l[k]instanceof a.Field))throw Error("Illegal reflect child of "+
this.toString(!0)+": "+l[k].toString(!0));return this.built=d};a.prototype.encode=function(b,c){for(var d=this.getChildren(a.Field),e=0;e<d.length;e++)d[e].encode(b.get(d[e].name),c);return c};a.prototype.decode=function(a,c){c=c||-1;for(var d=a.offset,e=new this.built;a.offset<d+c||-1==c&&0<a.remaining();){var f=a.readVarint32(),h=f&7,f=f>>3,m=this.getChild(f);if(!m)throw Error("Illegal field id in "+this.toString(!0)+"#decode: "+f);m.repeated&&!m.options.packed?e.add(m.name,m.decode(h,a)):e.set(m.name,
m.decode(h,a))}d=this.getChildren(b.Field);for(h=0;h<d.length;h++)if(d[h].required&&null===e[d[h].name])throw d=Error("Missing field "+d[h].toString(!0)+" in "+this.toString(!0)+"#decode"),d.msg=e,d;return e};b.Message=a;var d=function(a,b,c,d,e,h){f.call(this,a,d);this.required="required"==b;this.repeated="repeated"==b;this.type=c;this.id=e;this.options=h||{}};d.prototype=Object.create(f.prototype);d.prototype.verifyValue=function(a,b){b=b||!1;if(null===a){if(this.required)throw Error("Illegal value for "+
...
b.replace(/[\/\\][^\/\\]*$/,""),f=0;f<a.imports.length;f++){var h=e+"/"+a.imports[f];if(/\.json$/i.test(h)){var k=c.Util.fetch(h);if(null===k)throw Error("Failed to import '"+h+"' in '"+b+"': File not found");this["import"](JSON.parse(k),h)}else if(!/google\/protobuf\//.test(h)){k=c.Util.fetch(h);if(null===k)throw Error("Failed to import '"+h+"' in '"+b+"': File not found");k=new c.DotProto.Parser(k+"");this["import"](k.parse(),h)}}}return this};h.prototype.resolveAll=function(){if(!(null==this.ptr||
