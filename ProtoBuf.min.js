...
*/
(function(p){function q(n){if(!n||!n.zigZagEncode32)throw Error("ProtoBuf.js requires ByteBuffer.js >=1.1.0: Get it at https://github.com/dcodeIO/ByteBuffer.js");var f={VERSION:"1.0.0",WIRE_TYPES:{}};f.WIRE_TYPES.VARINT=0;f.WIRE_TYPES.BITS64=1;f.WIRE_TYPES.LDELIM=2;f.WIRE_TYPES.STARTGROUP=3;f.WIRE_TYPES.ENDGROUP=4;f.WIRE_TYPES.BITS32=5;f.TYPES={int32:{name:"int32",wireType:f.WIRE_TYPES.VARINT},uint32:{name:"uint32",wireType:f.WIRE_TYPES.VARINT},sint32:{name:"sint32",wireType:f.WIRE_TYPES.VARINT},
...
g.prototype.toBuffer=function(){return this.encode().toBuffer()};g.decode=function(a){a=a?a instanceof n?a:n.wrap(a):new n;var c=a.littleEndian,d=b.decode(a.LE());a.littleEndian=c;return d};g.prototype.toString=function(){return b.toString()};Object.defineProperty&&Object.defineProperty(g,"$options",{value:b.buildOpt(),enumerable:!1,configurable:!1,writable:!1});return g}(b,this),k=this.getChildren(),l=0;l<k.length;l++)if(k[l]instanceof e)d[k[l].name]=k[l].build();else if(k[l]instanceof a)d[k[l].name]=
k[l].build();else if(!(k[l]instanceof a.Field))throw Error("Illegal reflect child of "+this.toString(!0)+": "+k[l].toString(!0));return this.built=d};a.prototype.encode=function(c,b){for(var d=this.getChildren(a.Field),e=0;e<d.length;e++)d[e].encode(c.get(d[e].name),b);return b};a.prototype.decode=function(a,c){c=c||-1;for(var d=a.offset,e=new this.built;a.offset<d+c||-1==c&&0<a.remaining();){var f=a.readVarint32(),h=f&7,f=f>>3,m=this.getChild(f);if(!m)throw Error("Illegal field id in "+this.toString(!0)+
"#decode: "+f);m.repeated&&!m.options.packed?e.add(m.name,m.decode(h,a)):e.set(m.name,m.decode(h,a))}d=this.getChildren(b.Field);for(h=0;h<d.length;h++)if(d[h].required&&null===e[d[h].name])throw d=Error("Missing field "+d[h].toString(!0)+" in "+this.toString(!0)+"#decode"),d.msg=e,d;return e};b.Message=a;var d=function(a,c,b,d,e,h){f.call(this,a,d);this.required="required"==c;this.repeated="repeated"==c;this.type=b;this.id=e;this.options=h||{}};d.prototype=Object.create(f.prototype);d.prototype.verifyValue=
function(a,b){b=b||!1;if(null===a){if(this.required)throw Error("Illegal value for "+this.toString(!0)+": "+a+" (required)");return null}var d;if(this.repeated&&!b){a instanceof Array||(a=[a]);var f=[];for(d=0;d<a.length;d++)f.push(this.verifyValue(a[d],!0));return f}if(!this.repeated&&a instanceof Array)throw Error("Illegal value for "+this.toString(!0)+": "+a+" (no array expected)");if(this.type==c.TYPES.int32||this.type==c.TYPES.sint32||this.type==c.TYPES.sfixed32)return parseInt(a,10)|0;if(this.type==
c.TYPES.uint32||this.type==c.TYPES.fixed32)return parseInt(a,10)>>>0;if(c.Long){if(this.type==c.TYPES.int64||this.type==c.TYPES.sint64||this.type==c.TYPES.sfixed64)return!("object"==typeof a&&a instanceof c.Long)?c.Long.fromNumber(a,!1):a.unsigned?a.toSigned():a;if(this.type==c.TYPES.uint64||this.type==c.TYPES.fixed64)return!("object"==typeof a&&a instanceof c.Long)?c.Long.fromNumber(a,!0):a.unsigned?a:a.toUnsigned()}if(this.type==c.TYPES.bool)return!!a;if(this.type==c.TYPES["float"]||this.type==
c.TYPES["double"])return parseFloat(a);if(this.type==c.TYPES.string)return""+a;if(this.type==c.TYPES.bytes)return"object"==typeof a&&a instanceof n?a:n.wrap(a);if(this.type==c.TYPES["enum"]){f=this.resolvedType.getChildren(e.Value);for(d=0;d<f.length;d++)if(f[d].name==a||f[d].id==a)return f[d].id;throw Error("Illegal value for "+this.toString(!0)+": "+a+" (not a valid enum value)");}if(this.type==c.TYPES.message){if("object"!=typeof a)throw Error("Illegal value for "+this.toString(!0)+": "+a+" (object expected)");
return a instanceof this.resolvedType.built?a:new this.resolvedType.built(a)}throw Error("[INTERNAL ERROR] Illegal value for "+this.toString(!0)+": "+a+" (undefined type "+this.type+")");};d.prototype.encode=function(a,b){a=this.verifyValue(a);if(null==this.type||"object"!=typeof this.type)throw Error("[INTERNAL ERROR] Unresolved type in "+this.toString(!0)+": "+this.type);if(null===a||this.repeated&&0==a.length)return b;try{if(this.repeated){var d;if(this.options.packed){b.writeVarint32(this.id<<
...
Array))return!1;for(c=0;c<a.enums.length;c++)if(!h.isValidEnum(a.enums[c]))return!1}if("undefined"!=typeof a.messages){if(!(a.messages instanceof Array))return!1;for(c=0;c<a.messages.length;c++)if(!h.isValidMessage(a.messages[c]))return!1}return!0};h.isValidMessageField=function(a){if("string"!=typeof a.rule||"string"!=typeof a.name||"string"!=typeof a.type||"undefined"==typeof a.id||!b.RULE.test(a.rule)||!b.NAME.test(a.name)||!b.TYPEREF.test(a.type)||!b.ID.test(""+a.id))return!1;if("undefined"!=
typeof a.options){if("object"!=typeof a.options)return!1;for(var c=Object.keys(a.options),e=0;e<c.length;e++)if(!b.NAME.test(c[e])||"string"!=typeof a.options[c[e]]&&"number"!=typeof a.options[c[e]])return!1}return!0};h.isValidEnum=function(a){if("string"!=typeof a.name||!b.NAME.test(a.name)||"undefined"==typeof a.values||!(a.values instanceof Array)||0==a.values.length)return!1;for(var c=0;c<a.values.length;c++)if("object"!=typeof a.values[c]||"string"!=typeof a.values[c].name||"undefined"==typeof a.values[c].id||
!b.NAME.test(a.values[c].name)||!b.ID.test(""+a.values[c].id))return!1;return!0};h.prototype.create=function(a){if(a&&(a instanceof Array||(a=[a]),0!=a.length)){var c=[],e,g,k,l,n;for(c.push(a);0<c.length;){a=c.pop();if(a instanceof Array)for(;0<a.length;)if(e=a.shift(),h.isValidMessage(e)){g=new f.Message(this.ptr,e.name,e.options);if(e.fields&&0<e.fields.length)for(l=0;l<e.fields.length;l++){if(!h.isValidMessageField(e.fields[l]))throw Error("Not a valid message field definition in message "+g.name+
": "+JSON.stringify(e.fields[l]));if(g.hasChild(e.fields[l].id))throw Error("Duplicate field id in message "+g.name+": "+e.fields[l].id);if(e.fields[l].options){k=Object.keys(e.fields[l].options);for(n=0;n<k.length;n++){if(!b.NAME.test(k[n]))throw Error("Illegal field option name in message "+g.name+"#"+e.fields[l].name+": "+k[n]);if("string"!=typeof e.fields[l].options[k[n]]&&"number"!=typeof e.fields[l].options[k[n]])throw Error("Illegal field option value in message "+g.name+"#"+e.fields[l].name+
"#"+k[n]+": "+e.fields[l].options[k[n]]);}}g.addChild(new f.Message.Field(g,e.fields[l].rule,e.fields[l].type,e.fields[l].name,e.fields[l].id,e.fields[l].options))}k=[];if("undefined"!=typeof e.enums&&0<e.enums.length)for(l=0;l<e.enums.length;l++)k.push(e.enums[l]);if(e.messages&&0<e.messages.length)for(l=0;l<e.messages.length;l++)k.push(e.messages[l]);this.ptr.addChild(g);0<k.length&&(c.push(a),a=k,this.ptr=g)}else if(h.isValidEnum(e)){g=new f.Enum(this.ptr,e.name,e.options);for(l=0;l<e.values.length;l++)g.addChild(new f.Enum.Value(g,
e.values[l].name,e.values[l].id));this.ptr.addChild(g)}else throw Error("Not a valid message or enum definition: "+JSON.stringify(e));else throw Error("Not a valid namespace definition: "+JSON.stringify(a));this.ptr=this.ptr.parent}this.resolved=!1;this.result=null;return this}};h.isValidImport=function(a){return!/google\/protobuf\//.test(a)};h.prototype["import"]=function(a,b){a["package"]&&this.define(a["package"],a.options);a.messages&&this.create(a.messages);this.reset();a["package"]&&this.define(a["package"],
a.options);a.enums&&this.create(a.enums);this.reset();if(a.imports&&0<a.imports.length){if(!b)throw Error("Cannot determine import root: File name is unknown");for(var e=b.replace(/[\/\\][^\/\\]*$/,""),f=0;f<a.imports.length;f++){var k=e+"/"+a.imports[f];if(/\.json$/i.test(k)){var l=c.Util.fetch(k);if(null===l)throw Error("Failed to import '"+k+"' in '"+b+"': File not found");this["import"](JSON.parse(l),k)}else if(h.isValidImport(k)){l=c.Util.fetch(k);if(null===l)throw Error("Failed to import '"+
