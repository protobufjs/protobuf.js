...
*/
(function(p){function q(n){if(!n||!n.zigZagEncode32)throw Error("ProtoBuf.js requires ByteBuffer.js >=1.1.0: Get it at https://github.com/dcodeIO/ByteBuffer.js");var f={VERSION:"1.1.4",WIRE_TYPES:{}};f.WIRE_TYPES.VARINT=0;f.WIRE_TYPES.BITS64=1;f.WIRE_TYPES.LDELIM=2;f.WIRE_TYPES.STARTGROUP=3;f.WIRE_TYPES.ENDGROUP=4;f.WIRE_TYPES.BITS32=5;f.TYPES={int32:{name:"int32",wireType:f.WIRE_TYPES.VARINT},uint32:{name:"uint32",wireType:f.WIRE_TYPES.VARINT},sint32:{name:"sint32",wireType:f.WIRE_TYPES.VARINT},
...
c){this.parent=a;this.name=c};f.prototype.toString=function(c){var k=this.name,d=this;do{d=d.parent;if(null==d)break;k=d.name+"."+k}while(1);c&&(this instanceof b.Message?k="Message "+k:this instanceof a.Field?k="Message.Field "+k:this instanceof e?k="Enum "+k:this instanceof e.Value?k="Enum.Value "+k:this instanceof h&&(k="Namespace "+k));return k};f.prototype.build=function(){throw Error(this.toString(!0)+" cannot be built directly");};b.T=f;var h=function(a,c,b){f.call(this,a,c);this.children=
...
function(a){return a.toUpperCase().replace("_","")}),c=c.substring(0,1).toUpperCase()+c.substring(1),d=a.name.replace(/([A-Z])/g,function(a){return"_"+a});b.hasChild("set"+c)||(g.prototype["set"+c]=function(c){this.set(a.name,c)});b.hasChild("set_"+d)||(g.prototype["set_"+d]=function(c){this.set(a.name,c)});b.hasChild("get"+c)||(g.prototype["get"+c]=function(){return this.get(a.name)});b.hasChild("get_"+d)||(g.prototype["get_"+d]=function(){return this.get(a.name)})})(d[k]);g.prototype.encode=function(a){a=
...
Object.defineProperty(g,"$options",{value:b.buildOpt(),enumerable:!1,configurable:!1,writable:!1});return g}(b,this),k=this.getChildren(),l=0;l<k.length;l++)if(k[l]instanceof e)d[k[l].name]=k[l].build();else if(k[l]instanceof a)d[k[l].name]=k[l].build();else if(!(k[l]instanceof a.Field))throw Error("Illegal reflect child of "+this.toString(!0)+": "+k[l].toString(!0));return this.clazz=d};a.prototype.encode=function(c,b){for(var d=this.getChildren(a.Field),e=0;e<d.length;e++)d[e].encode(c.get(d[e].name),
b);return b};a.prototype.decode=function(a,d){d="number"===typeof d?d:-1;for(var e=a.offset,f=new this.clazz;a.offset<e+d||-1==d&&0<a.remaining();){var h=a.readVarint32(),m=h&7,h=h>>3,n=this.getChild(h);if(n)n.repeated&&!n.options.packed?f.add(n.name,n.decode(m,a)):f.set(n.name,n.decode(m,a));else switch(m){case c.WIRE_TYPES.VARINT:a.readVarint32();break;case c.WIRE_TYPES.BITS32:a.offset+=4;break;case c.WIRE_TYPES.BITS64:a.offset+=8;break;case c.WIRE_TYPES.LDELIM:m=a.readVarint32();a.offset+=m;break;
default:throw Error("Illegal wire type of unknown field "+h+" in "+this.toString(!0)+"#decode: "+m);}}e=this.getChildren(b.Field);for(m=0;m<e.length;m++)if(e[m].required&&null===f[e[m].name])throw e=Error("Missing field "+e[m].toString(!0)+" in "+this.toString(!0)+"#decode"),e.msg=f,e;return f};b.Message=a;var d=function(a,c,b,d,e,h){f.call(this,a,d);this.required="required"==c;this.repeated="repeated"==c;this.type=b;this.resolvedType=null;this.id=e;this.options=h||{}};d.prototype=Object.create(f.prototype);
d.prototype.verifyValue=function(a,b){b=b||!1;if(null===a){if(this.required)throw Error("Illegal value for "+this.toString(!0)+": "+a+" (required)");return null}var d;if(this.repeated&&!b){a instanceof Array||(a=[a]);var f=[];for(d=0;d<a.length;d++)f.push(this.verifyValue(a[d],!0));return f}if(!this.repeated&&a instanceof Array)throw Error("Illegal value for "+this.toString(!0)+": "+a+" (no array expected)");if(this.type==c.TYPES.int32||this.type==c.TYPES.sint32||this.type==c.TYPES.sfixed32)return parseInt(a,
10)|0;if(this.type==c.TYPES.uint32||this.type==c.TYPES.fixed32)return parseInt(a,10)>>>0;if(c.Long){if(this.type==c.TYPES.int64||this.type==c.TYPES.sint64||this.type==c.TYPES.sfixed64)return!("object"==typeof a&&a instanceof c.Long)?c.Long.fromNumber(a,!1):a.unsigned?a.toSigned():a;if(this.type==c.TYPES.uint64||this.type==c.TYPES.fixed64)return!("object"==typeof a&&a instanceof c.Long)?c.Long.fromNumber(a,!0):a.unsigned?a:a.toUnsigned()}if(this.type==c.TYPES.bool)return!!a;if(this.type==c.TYPES["float"]||
this.type==c.TYPES["double"])return parseFloat(a);if(this.type==c.TYPES.string)return""+a;if(this.type==c.TYPES.bytes)return"object"==typeof a&&a instanceof n?a:n.wrap(a);if(this.type==c.TYPES["enum"]){f=this.resolvedType.getChildren(e.Value);for(d=0;d<f.length;d++)if(f[d].name==a||f[d].id==a)return f[d].id;throw Error("Illegal value for "+this.toString(!0)+": "+a+" (not a valid enum value)");}if(this.type==c.TYPES.message){if("object"!=typeof a)throw Error("Illegal value for "+this.toString(!0)+
...
!b.NEGID.test(""+a.values[c].id))return!1;return!0};h.prototype.create=function(a){if(a&&(a instanceof Array||(a=[a]),0!=a.length)){var c=[],e,g,k,l,n;for(c.push(a);0<c.length;){a=c.pop();if(a instanceof Array)for(;0<a.length;)if(e=a.shift(),h.isValidMessage(e)){g=new f.Message(this.ptr,e.name,e.options);if(e.fields&&0<e.fields.length)for(l=0;l<e.fields.length;l++){if(!h.isValidMessageField(e.fields[l]))throw Error("Not a valid message field definition in message "+g.name+": "+JSON.stringify(e.fields[l]));
if(g.hasChild(e.fields[l].id))throw Error("Duplicate field id in message "+g.name+": "+e.fields[l].id);if(e.fields[l].options){k=Object.keys(e.fields[l].options);for(n=0;n<k.length;n++){if(!b.NAME.test(k[n]))throw Error("Illegal field option name in message "+g.name+"#"+e.fields[l].name+": "+k[n]);if("string"!=typeof e.fields[l].options[k[n]]&&"number"!=typeof e.fields[l].options[k[n]])throw Error("Illegal field option value in message "+g.name+"#"+e.fields[l].name+"#"+k[n]+": "+e.fields[l].options[k[n]]);
}}g.addChild(new f.Message.Field(g,e.fields[l].rule,e.fields[l].type,e.fields[l].name,e.fields[l].id,e.fields[l].options))}k=[];if("undefined"!=typeof e.enums&&0<e.enums.length)for(l=0;l<e.enums.length;l++)k.push(e.enums[l]);if(e.messages&&0<e.messages.length)for(l=0;l<e.messages.length;l++)k.push(e.messages[l]);this.ptr.addChild(g);0<k.length&&(c.push(a),a=k,this.ptr=g)}else if(h.isValidEnum(e)){g=new f.Enum(this.ptr,e.name,e.options);for(l=0;l<e.values.length;l++)g.addChild(new f.Enum.Value(g,e.values[l].name,
e.values[l].id));this.ptr.addChild(g)}else throw Error("Not a valid message or enum definition: "+JSON.stringify(e));else throw Error("Not a valid namespace definition: "+JSON.stringify(a));this.ptr=this.ptr.parent}this.resolved=!1;this.result=null;return this}};h.isValidImport=function(a){return!/google\/protobuf\//.test(a)};h.prototype.files=[];h.prototype["import"]=function(a,b){if("string"===typeof b){c.Util.IS_NODE&&(b=require("path").resolve(b));if(this.files[b])return this.reset(),this;this.files[b]=
!0}a["package"]&&this.define(a["package"],a.options);a.messages&&this.create(a.messages);this.reset();a["package"]&&this.define(a["package"],a.options);a.enums&&this.create(a.enums);this.reset();if(a.imports&&0<a.imports.length){if(!b)throw Error("Cannot determine import root: File name is unknown");for(var e=b.replace(/[\/\\][^\/\\]*$/,""),f=0;f<a.imports.length;f++){var k=e+"/"+a.imports[f];if(/\.json$/i.test(k)){var l=c.Util.fetch(k);if(null===l)throw Error("Failed to import '"+k+"' in '"+b+"': File not found");
this["import"](JSON.parse(l),k)}else if(h.isValidImport(k)){l=c.Util.fetch(k);if(null===l)throw Error("Failed to import '"+k+"' in '"+b+"': File not found");l=new c.DotProto.Parser(l+"");this["import"](l.parse(),k)}}}return this};h.prototype.resolveAll=function(){if(!(null==this.ptr||"object"==typeof this.ptr.type)){if(this.ptr instanceof f.Namespace)for(var a=this.ptr.getChildren(),d=0;d<a.length;d++)this.ptr=a[d],this.resolveAll();else if(this.ptr instanceof f.Message.Field)if(b.TYPE.test(this.ptr.type))this.ptr.type=
