...
tape.test("longbits", function(test) {
    ...
    test.test(test.name + " - zero", function(test) {
        ...
        test.equal(protobuf.util.longFromHash("\0\0\0\0\0\0\0\0"), 0n, "should be returned for a binary hash of 8x0 (bigint 0 through util.longFromHash)");
        test.end();
    });
    ...
    .forEach(function(value) {
        ...
        test.equal(protobuf.util.longFromHash(comp, long.unsigned), BigInt(long.toString()), long + " should convert back to an equal bigint through util.longFromHash");
    });
    ...
    .forEach(function(long) {
        test.same(LongBits.from(long.toString()), LongBits.from(long), long + " should parse decimal strings exactly");
    });
